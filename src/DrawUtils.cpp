...
sf::RectangleShape DrawUtils::add_pixel(sf::Vector2f position, std::uint8_t red, std::uint8_t green, std::uint8_t blue) {
	...
	pixel.setScale({static_cast<float>(DrawUtils::SCALE), static_cast<float>(DrawUtils::SCALE)});
	return pixel;

}

std::vector<sf::RectangleShape> DrawUtils::draw_display_buffer(const uint8_t(&display)[Chip8::DISPLAY_WIDTH * Chip8::DISPLAY_HEIGHT]) {
	// TODO: see if there's a way to avoid redrawing everything
	std::vector<sf::RectangleShape> pixels{};

	for (unsigned int i = 0; i < sizeof(display); ++i) {
		if (display[i] == 1) {
			int x = i % Chip8::DISPLAY_WIDTH;
			int y = i / Chip8::DISPLAY_WIDTH;
			pixels.push_back(DrawUtils::add_pixel({ static_cast<float>(x), static_cast<float>(y) }, 255, 255, 255));
		}
	}
	return pixels;
}
